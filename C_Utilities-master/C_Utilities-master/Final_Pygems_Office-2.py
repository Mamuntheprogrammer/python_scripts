		...
		if not xllis:
			...
			Sub Test()
			    ...
			    Do While FileName <> vbNullString
			        ...
			        With ws
			            If .FilterMode Then .ShowAllData
			            If ShtCnt > 1 Then Range("A1").CurrentRegion.Copy Destination:=Merged.Worksheets(1).Cells(RowCnt, 1)
			        End With
			        wb.Close SaveChanges:=False
			        RowCnt = Application.WorksheetFunction.CountA(Merged.Worksheets(1).Columns("A:A")) + 1
			        FileName = Dir
...
s_label.pack(pady=20,side=LEFT)
